...
export const toISOStringZulu = (date: Date) => formatInTimeZone(date, 'UTC', "yyyy-MM-dd'T'HH:mm:ss'Z'");
...
// This is used when dealing with observation dates. These dates are given to us with no time zone
// or timestamp. By setting the time zone to Anchorage, we're creating a buffer for the user so that
// localized date for the user anywhere east of Anchorage will show the same day. The issue with
// setting it to Pacific time is that when toDate() is called, the Date object is set to midnight in
// the given time zone. That means that users in Alaska see the date as 1 day before the original
// date after localization is done because they're 1 hour behind.
export const observationDateToLocalShortDateString = (date: Date | string | undefined | null): string => {
  if (date == null) {
    return 'Unknown';
  }
...
// the given time zone. That means that users in Alaska see the date as 1 day before the original
// date after localization is done because they're 1 hour behind.
export const observationDateToLocalDateString = (date: Date | string | undefined | null): string => {
  if (date == null) {
    return 'Unknown';
  }
