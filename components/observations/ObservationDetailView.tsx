...
export const ObservationCard: React.FunctionComponent<{
  ...
  const url = 'https://' + observation.center_id + '.us/observations/#/view/observations/' + observation.id;
  const onShare = useCallback(
    () => async () => {
      try {
        ...
        logger.warn({error}, `failed to open the sharing modal`);
      }
    },
    [url],
  );
              ...
              <Card
                ...
                header={
                  <VStack space={8} width="100%">
                    ...
                    <BodySemibold>{observation.center_id}</BodySemibold>
                  </VStack>
                }>
